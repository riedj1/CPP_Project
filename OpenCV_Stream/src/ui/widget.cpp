...
void Widget::on_pushButton_close_Webcam_clicked()
{
    ...
    qt_image = QImage(static_cast<unsigned char*> (image.data), image.cols, image.rows, QImage::Format_RGB888);
...
void Widget::update_window()
{
    ...
    if(!frame_w.empty())
    {
        qt_image = QImage(static_cast<unsigned char*> (frame_w.data), frame_w.cols, frame_w.rows, QImage::Format_RGB888);
        ui->gui_window->setPixmap(QPixmap::fromImage(qt_image));
        ui->gui_window->resize(ui->gui_window->pixmap()->size());
    }
}
...
void Widget::canny_edge()
{
    ...
    qt_image = QImage(static_cast<unsigned char*> (frame_w.data), frame_w.cols, frame_w.rows, QImage::Format_Grayscale8);
    ui->gui_window->setPixmap(QPixmap::fromImage(qt_image));
    ui->gui_window->resize(ui->gui_window->pixmap()->size());
}
...
// checkbox: activate access on RGB
...
void Widget::on_verticalSlider_R_valueChanged(){ slider = new QSlider(Qt::Vertical); }

// slider: adjust color G
void Widget::on_verticalSlider_G_valueChanged(){ slider = new QSlider(Qt::Vertical); }

// slider: adjust color B
void Widget::on_verticalSlider_B_valueChanged(){ slider = new QSlider(Qt::Vertical); }
