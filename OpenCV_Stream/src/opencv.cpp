...
{
    cv::Mat _output = get();
    cv::cvtColor(_output, _output, CV_BGR2RGB);
    return _output;
}
...
{
    cv::Mat _output = get();
    cv::cvtColor(_output, _output, CV_BGR2GRAY);
    return _output;
}
...
cv::Mat OpenCV::getFaceFrame(cv::Mat _frame, float circle_dia)
{
    ...
    // Detect faces
    face_cascade.detectMultiScale(_frame, faces, 1.1, 2, 0|CV_HAAR_SCALE_IMAGE, cv::Size(140, 140));

    //Show the results
    for(uint i = 0; i < static_cast<uint>(faces.size()); i++)
    {
        cv::Point center(static_cast<int>(faces[i].x) + static_cast<int>(faces[i].width*0.5), static_cast<int>(faces[i].y + faces[i].height*0.5));
        ellipse(_frame, center, cv::Size(static_cast<int>(faces[i].width*circle_dia), static_cast<int>(faces[i].height*circle_dia)), 0, 0, 360, cv::Scalar(0, 0, 0), 4, 8, 0);
    }
    return _frame;
}
